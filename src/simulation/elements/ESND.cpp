...
void Element::Element_ESND()
{
	Identifier = "DEFAULT_PT_ESND";
	Name = "ESND";
	...
	HeatConduct = 150;
	Description = "Like sand, but explosive.";
...
static int update(UPDATE_FUNC_ARGS)
{
	...
	else //if (parts[i].tmp>=2)
	{
		...
		int n;
		unsigned col = (((esnd_data[caddress])) << 16) | (((esnd_data[caddress + 1])) << 8) | ((esnd_data[caddress + 2]));
		for (n = 0; n < 70; n++)
		{
			int num = RNG::Ref().between(0, 10);
			if (num != 0 && num != 1)
			{
			...
			{
				np = sim->create_part(-3, x, y, PT_BANG);
				if (np > -1)
				{
					magnitude = RNG::Ref().between(40, 99) * 0.05f;
					angle = RNG::Ref().between(0, 6283) * 0.001f;//(in radians, between 0 and 2*pi)
			...
			{
				np = sim->create_part(-3, x, y, PT_PHOT);
				if (np > -1)
				{
					magnitude = RNG::Ref().between(40, 99) * 0.05f;
					angle = RNG::Ref().between(0, 6283) * 0.001f;//(in radians, between 0 and 2*pi)
