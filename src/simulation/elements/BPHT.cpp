...
void Element::Element_BPHT()
{
	Identifier = "DEFAULT_PT_BPHT";
	Name = "BPHT";
	Colour = 0xFFFFFF_rgb;
	MenuVisible = 1;
	...
	HeatConduct = 251;
	Description = "Photons. But bouncy! Also not hot!";
		...
		for (auto ry = -1; ry <= 1; ry++)
		{
			...
			if (TYP(r)==PT_ISOZ || TYP(r)==PT_ISZS)
			{
				if (sim->rng.chance(1, 400))
				{
					parts[i].vx *= 0.90f;
					parts[i].vy *= 0.90f;
					sim->create_part(ID(r), x+rx, y+ry, PT_BPHT);
